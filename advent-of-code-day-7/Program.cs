...
namespace advent_of_code_day_7
{
    internal class Program
    {
        ...
        /// <returns>Signal value of wire a</returns>
        private static string SolvePuzzle(IList<string> instructions)
        {
            // Define our circuit
            var circuit = new Dictionary<string, ushort>();
            ...
            var i = 0;
            var exception = false;
            while (circuit.Count < instructionCount)
            {
                try
                {
                    var si = instructions[i].Split(' ');

                    if (si[1] == "->")
                    {
                        circuit.Add(si[si.Length - 1], si[0].All(char.IsLetter) ? circuit[si[0]] : ushort.Parse(si[0]));
                    }
                    else if (si[1] == "AND")
                    {
                        circuit.Add(si[si.Length - 1],
                            (ushort)
                                ((si[0].All(char.IsLetter) ? circuit[si[0]] : ushort.Parse(si[0])) &
                                 (si[2].All(char.IsLetter) ? circuit[si[2]] : ushort.Parse(si[2]))));
                    }
                    else if (si[1] == "OR")
                    {
                        circuit.Add(si[si.Length - 1],
                    ...
                    else if (si[1] == "LSHIFT")
                    {
                        circuit.Add(si[si.Length - 1],
                            (ushort)
                                ((si[0].All(char.IsLetter) ? circuit[si[0]] : ushort.Parse(si[0])) << short.Parse(si[2])));
                    }
                    else if (si[1] == "RSHIFT")
                    {
                        circuit.Add(si[si.Length - 1],
                    ...
                    {
                        circuit.Add(si[si.Length - 1],
                            (ushort)~(si[1].All(char.IsLetter) ? circuit[si[1]] : ushort.Parse(si[1])));
                    }

                    Console.WriteLine("Wire added to circuit.Total wires in circuit: {0}. Removed instruction[{1}]", circuit.Count, instructions[i]);
                    exception = false;
                }
                ...
                finally
                {
                    if (!exception)
                    {
                        // Remove instruction from the list if the signal calculated succesfully
                        instructions.Remove(instructions[i]);
                    }

                    if (i >= instructions.Count - 1)
                    {
                        // Start over if reached to the end of instructions
                        i = 0;
                    }
                    else
                    {
                        if (exception)
                        {
                            // Move to the next item if KeyNotFoundException occurred
                            i++;
                        }
                    }
                }
            ...
            return string.Format("Wire: [a], Signal: [{0}]", circuit["a"]);
        }
    }
}
